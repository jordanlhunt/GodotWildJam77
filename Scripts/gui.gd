...
@onready var nextShapeGridNode := get_node("PanelContainer/HBoxContainer/LeftSide/VBox/NextShapeContainer/NextShapeGrid")
...
func set_next_shape(next_shape: ShapeData) -> void:
	clear_cells(nextShapeGridNode)
	var i: int = 0
	for column in next_shape.coordinates.size():
		for row in [0, 1]:
			if next_shape.grid[row][column]:
				nextShapeGridNode.get_child(i).modulate = next_shape.color
			i += 1
